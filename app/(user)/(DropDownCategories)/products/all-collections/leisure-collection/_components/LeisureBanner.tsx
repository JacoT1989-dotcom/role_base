...
const COLLECTION_ID = "leisure-collection";
const COLLECTION_TITLE = "Leisure Collection";

const LeisureCollection: React.FC = () => {
  ...
  const leisureCollectionBanner = useMemo(
  ...
  const handleBannerUpdate = async (formData: FormData) => {
    if (!leisureCollectionBanner) return;

    try {
      await updateWithImage(leisureCollectionBanner.id, formData);
    } catch (error) {
  ...
  if (!leisureCollectionBanner && isEditor) {
  ...
  if (leisureCollectionBanner) {
    return (
      <UniversalHeroSection
        heroData={leisureCollectionBanner}
        type="collection"
...
export default LeisureCollection;
