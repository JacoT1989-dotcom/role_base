...
const COLLECTION_ID = "industrial-collection";
const COLLECTION_TITLE = "Industrial Collection";

const IndustrialCollection: React.FC = () => {
  ...
  const industrialCollectionBanner = useMemo(
  ...
  const handleBannerUpdate = async (formData: FormData) => {
    if (!industrialCollectionBanner) return;

    try {
      await updateWithImage(industrialCollectionBanner.id, formData);
    } catch (error) {
  ...
  if (!industrialCollectionBanner && isEditor) {
  ...
  if (industrialCollectionBanner) {
    return (
      <UniversalHeroSection
        heroData={industrialCollectionBanner}
        type="collection"
...
export default IndustrialCollection;
