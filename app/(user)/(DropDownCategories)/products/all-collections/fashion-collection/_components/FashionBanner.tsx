...
const COLLECTION_ID = "fashion-collection";
const COLLECTION_TITLE = "Fashion Collection";
...
const FashionCollection: React.FC = () => {
  ...
  const fashionCollectionBanner = useMemo(
  ...
  const handleBannerUpdate = async (formData: FormData) => {
    if (!fashionCollectionBanner) return;

    try {
      await updateWithImage(fashionCollectionBanner.id, formData);
    } catch (error) {
  ...
  if (!fashionCollectionBanner && isEditor) {
  ...
  if (fashionCollectionBanner) {
    return (
      <UniversalHeroSection
        heroData={fashionCollectionBanner}
        type="collection"
