...
const COLLECTION_ID = "winter-collection";
const COLLECTION_TITLE = "Winter Collection";
...
const WinterCollection: React.FC = () => {
  ...
  const winterCollectionBanner = useMemo(
  ...
  const handleBannerUpdate = async (formData: FormData) => {
    if (!winterCollectionBanner) return;

    try {
      await updateWithImage(winterCollectionBanner.id, formData);
    } catch (error) {
  ...
  if (!winterCollectionBanner && isEditor) {
  ...
  if (winterCollectionBanner) {
    return (
      <UniversalHeroSection
        heroData={winterCollectionBanner}
        type="collection"
