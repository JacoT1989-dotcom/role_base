...
const COLLECTION_ID = "camo-collection";
const COLLECTION_TITLE = "Camo Collection";

const CamoCollection: React.FC = () => {
  ...
  const camoCollectionBanner = useMemo(
  ...
  const handleBannerUpdate = async (formData: FormData) => {
    if (!camoCollectionBanner) return;

    try {
      await updateWithImage(camoCollectionBanner.id, formData);
    } catch (error) {
  ...
  if (!camoCollectionBanner && isEditor) {
  ...
  if (camoCollectionBanner) {
    return (
      <UniversalHeroSection
        heroData={camoCollectionBanner}
        type="collection"
