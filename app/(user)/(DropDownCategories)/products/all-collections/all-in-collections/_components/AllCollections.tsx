...
const COLLECTION_ID = "all-collections";
const COLLECTION_TITLE = "All Collections";
...
const AllCollections: React.FC = () => {
  ...
  const allCollectionsBanner = useMemo(
  ...
  const handleBannerUpdate = async (formData: FormData) => {
    if (!allCollectionsBanner) return;

    try {
      await updateWithImage(allCollectionsBanner.id, formData);
    } catch (error) {
  ...
  if (!allCollectionsBanner && isEditor) {
  ...
  if (allCollectionsBanner) {
    return (
      <UniversalHeroSection
        heroData={allCollectionsBanner}
        type="collection"
