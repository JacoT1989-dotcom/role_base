...
const COLLECTION_ID = "sport-collection";
const COLLECTION_TITLE = "Sport Collection";
...
const SportCollection: React.FC = () => {
  ...
  const sportCollectionBanner = useMemo(
  ...
  const handleBannerUpdate = async (formData: FormData) => {
    if (!sportCollectionBanner) return;

    try {
      await updateWithImage(sportCollectionBanner.id, formData);
    } catch (error) {
  ...
  if (!sportCollectionBanner && isEditor) {
  ...
  if (sportCollectionBanner) {
    return (
      <UniversalHeroSection
        heroData={sportCollectionBanner}
        type="collection"
