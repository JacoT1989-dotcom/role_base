...
const COLLECTION_ID = "baseball-collection";
const COLLECTION_TITLE = "Baseball Collection";

const BaseballCollection: React.FC = () => {
  ...
  const baseballCollectionBanner = useMemo(
  ...
  const handleBannerUpdate = async (formData: FormData) => {
    if (!baseballCollectionBanner) return;

    try {
      await updateWithImage(baseballCollectionBanner.id, formData);
    } catch (error) {
  ...
  if (!baseballCollectionBanner && isEditor) {
  ...
  if (baseballCollectionBanner) {
    return (
      <UniversalHeroSection
        heroData={baseballCollectionBanner}
        type="collection"
