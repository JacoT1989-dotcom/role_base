...
const COLLECTION_ID = "kids-collection";
const COLLECTION_TITLE = "Kids Collection";
...
const KidsCollection: React.FC = () => {
  ...
  const kidsCollectionBanner = useMemo(
  ...
  const handleBannerUpdate = async (formData: FormData) => {
    if (!kidsCollectionBanner) return;

    try {
      await updateWithImage(kidsCollectionBanner.id, formData);
    } catch (error) {
  ...
  if (!kidsCollectionBanner && isEditor) {
  ...
  if (kidsCollectionBanner) {
    return (
      <UniversalHeroSection
        heroData={kidsCollectionBanner}
        type="collection"
