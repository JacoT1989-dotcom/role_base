...
const COLLECTION_ID = "signature-collection";
const COLLECTION_TITLE = "Signature Collection";
...
const SignatureCollection: React.FC = () => {
  ...
  const signatureCollectionBanner = useMemo(
  ...
  const handleBannerUpdate = async (formData: FormData) => {
    if (!signatureCollectionBanner) return;

    try {
      await updateWithImage(signatureCollectionBanner.id, formData);
    } catch (error) {
  ...
  if (!signatureCollectionBanner && isEditor) {
  ...
  if (signatureCollectionBanner) {
    return (
      <UniversalHeroSection
        heroData={signatureCollectionBanner}
        type="collection"
