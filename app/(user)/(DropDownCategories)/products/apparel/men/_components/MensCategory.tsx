...
const MensCategory: React.FC = () => {
  ...
  const mensHero = useMemo(
    () => heroes.find(hero => hero.categoryName === "MENS"),
    [heroes]
  );

  const handleNewUpload = async (formData: FormData) => {
    try {
      formData.set("title", "MENS");
      formData.set("categoryName", "MENS");
  ...
  return <HeroSection heroData={mensHero} />;
};
