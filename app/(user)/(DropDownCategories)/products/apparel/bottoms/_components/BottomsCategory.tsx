...
const BottomsCategory: React.FC = () => {
  ...
  const bottomsHero = useMemo(
    () => heroes.find(hero => hero.categoryName === "BOTTOMS"),
    [heroes]
  );

  const handleNewUpload = async (formData: FormData) => {
    try {
      formData.set("title", "BOTTOMS");
      formData.set("categoryName", "BOTTOMS");
  ...
  if (!bottomsHero && isEditor) {
    return (
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        <EmptyHeroUpload
          onUpload={handleNewUpload}
          defaultTitle="BOTTOMS"
  ...
  return <HeroSection heroData={bottomsHero} />;
};

export default BottomsCategory;
