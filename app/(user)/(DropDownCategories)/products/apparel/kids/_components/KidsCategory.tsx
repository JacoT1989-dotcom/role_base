...
const KidsCategory: React.FC = () => {
  ...
  const kidsHero = useMemo(
    () => heroes.find(hero => hero.categoryName === "KIDS"),
    [heroes]
  );

  const handleNewUpload = async (formData: FormData) => {
    try {
      formData.set("title", "KIDS");
      formData.set("categoryName", "KIDS");
  ...
  return <HeroSection heroData={kidsHero} />;
};
