...
const HoodiesCategory: React.FC = () => {
  ...
  const hoodiesHero = useMemo(
    () => heroes.find(hero => hero.categoryName === "HOODIES"),
    [heroes]
  );

  const handleNewUpload = async (formData: FormData) => {
    try {
      formData.set("title", "HOODIES");
      formData.set("categoryName", "HOODIES");
  ...
  if (!hoodiesHero && isEditor) {
    return (
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        <EmptyHeroUpload
          onUpload={handleNewUpload}
          defaultTitle="HOODIES"
  ...
  return <HeroSection heroData={hoodiesHero} />;
};
