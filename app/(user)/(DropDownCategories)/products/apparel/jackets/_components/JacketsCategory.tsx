...
const JacketsCategory: React.FC = () => {
  ...
  const jacketsHero = useMemo(
    () => heroes.find(hero => hero.categoryName === "JACKETS"),
    [heroes]
  );

  const handleNewUpload = async (formData: FormData) => {
    try {
      formData.set("title", "JACKETS");
      formData.set("categoryName", "JACKETS");
  ...
  if (!jacketsHero && isEditor) {
    return (
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        <EmptyHeroUpload
          onUpload={handleNewUpload}
          defaultTitle="JACKETS"
          defaultCategory="JACKETS"
        />
      </div>
    );
  }

  return <HeroSection heroData={jacketsHero} />;
};

export default JacketsCategory;
