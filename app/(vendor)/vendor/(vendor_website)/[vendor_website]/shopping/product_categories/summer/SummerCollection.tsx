...
const VendorSummerCollection: React.FC = () => {
  ...
  if (!displayCollections?.summer || displayCollections.summer.length === 0) {
    return (
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(displayCollections.summer.length / ITEMS_PER_PAGE)
  );
  const safeCurrentPage = Math.min(currentPage, totalPages);
  const startIndex = (safeCurrentPage - 1) * ITEMS_PER_PAGE;
  const currentProducts = displayCollections.summer.slice(
    startIndex,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="text-sm text-muted-foreground text-center mt-4">
        ...
        {Math.min(
          startIndex + ITEMS_PER_PAGE,
          displayCollections.summer.length
        )}{" "}
        of {displayCollections.summer.length} products
      </div>
    </div>
  );
};
