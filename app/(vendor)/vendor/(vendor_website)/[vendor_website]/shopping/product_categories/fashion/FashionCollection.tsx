...
const VendorFashionCollection: React.FC = () => {
  ...
  if (!displayCollections?.fashion || displayCollections.fashion.length === 0) {
    return (
      <div className="text-center py-8">
        <h2 className="text-2xl font-bold text-foreground">
          No products found in fashion collection.
        </h2>
        {user?.role === "VENDOR" && (
          <p className="text-muted-foreground mt-2">
            Add some fashion products to see them here.
          </p>
        )}
      </div>
    );
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(displayCollections.fashion.length / ITEMS_PER_PAGE)
  );
  const safeCurrentPage = Math.min(currentPage, totalPages);
  const startIndex = (safeCurrentPage - 1) * ITEMS_PER_PAGE;
  const currentProducts = displayCollections.fashion.slice(
    startIndex,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="text-sm text-muted-foreground text-center mt-4">
        ...
        {Math.min(
          startIndex + ITEMS_PER_PAGE,
          displayCollections.fashion.length
        )}{" "}
        of {displayCollections.fashion.length} products
      </div>
    </div>
  );
};

export default VendorFashionCollection;
