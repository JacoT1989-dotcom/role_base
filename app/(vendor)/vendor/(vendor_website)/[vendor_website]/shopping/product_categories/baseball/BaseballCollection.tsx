...
const VendorBaseballCollection: React.FC = () => {
  ...
  if (
    !displayCollections?.baseball ||
    displayCollections.baseball.length === 0
  ) {
    return (
      <div className="text-center py-8">
        <h2 className="text-2xl font-bold text-foreground">
          No products found in baseball collection.
        </h2>
        {user?.role === "VENDOR" && (
          <p className="text-muted-foreground mt-2">
            Add some baseball products to see them here.
          </p>
        )}
      </div>
    );
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(displayCollections.baseball.length / ITEMS_PER_PAGE)
  );
  const safeCurrentPage = Math.min(currentPage, totalPages);
  const startIndex = (safeCurrentPage - 1) * ITEMS_PER_PAGE;
  const currentProducts = displayCollections.baseball.slice(
    startIndex,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="text-sm text-muted-foreground text-center mt-4">
        ...
        {Math.min(
          startIndex + ITEMS_PER_PAGE,
          displayCollections.baseball.length
        )}{" "}
        of {displayCollections.baseball.length} products
      </div>
    </div>
  );
};

export default VendorBaseballCollection;
