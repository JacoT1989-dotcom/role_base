...
import { useCollectionInitialization } from "../_store/useCollectionInitialization";

const ITEMS_PER_PAGE = 4;

const VendorAfricanCollection: React.FC = () => {
  const params = useParams();
  const vendorWebsite = params?.vendor_website as string;
  const { user } = useSession();
  const [currentPage, setCurrentPage] = useState(1);
  const { collections, filteredCollections } = useVendorCollectionsStore();
  const { isLoading, error } = useCollectionInitialization({
    user,
    vendorWebsite,
  });
  ...
  if (!displayCollections?.african || displayCollections.african.length === 0) {
    return (
      <div className="text-center py-8">
        <h2 className="text-2xl font-bold text-foreground">
          No products found in African collection.
        </h2>
        {user?.role === "VENDOR" && (
          <p className="text-muted-foreground mt-2">
            Add some African products to see them here.
          </p>
        )}
      </div>
    );
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(displayCollections.african.length / ITEMS_PER_PAGE)
  );
  const safeCurrentPage = Math.min(currentPage, totalPages);
  const startIndex = (safeCurrentPage - 1) * ITEMS_PER_PAGE;
  const currentProducts = displayCollections.african.slice(
    startIndex,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="text-sm text-muted-foreground text-center mt-4">
        ...
        {Math.min(
          startIndex + ITEMS_PER_PAGE,
          displayCollections.african.length
        )}{" "}
        of {displayCollections.african.length} products
      </div>
    </div>
  );
};

export default VendorAfricanCollection;
