...
const VendorSignatureCollection: React.FC = () => {
  ...
  if (
    !displayCollections?.signature ||
    displayCollections.signature.length === 0
  ) {
    return (
      <div className="text-center py-8">
        <h2 className="text-2xl font-bold text-foreground">
          No products found in signature collection.
        </h2>
        {user?.role === "VENDOR" && (
          <p className="text-muted-foreground mt-2">
            Add some signature products to see them here.
          </p>
        )}
      </div>
    );
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(displayCollections.signature.length / ITEMS_PER_PAGE)
  );
  const safeCurrentPage = Math.min(currentPage, totalPages);
  const startIndex = (safeCurrentPage - 1) * ITEMS_PER_PAGE;
  const currentProducts = displayCollections.signature.slice(
    startIndex,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="text-sm text-muted-foreground text-center mt-4">
        ...
        {Math.min(
          startIndex + ITEMS_PER_PAGE,
          displayCollections.signature.length
        )}{" "}
        of {displayCollections.signature.length} products
      </div>
    </div>
  );
};

export default VendorSignatureCollection;
