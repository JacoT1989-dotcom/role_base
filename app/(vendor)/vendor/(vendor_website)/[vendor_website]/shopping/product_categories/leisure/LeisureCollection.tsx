...
const VendorLeisureCollection: React.FC = () => {
  ...
  if (!displayCollections?.leisure || displayCollections.leisure.length === 0) {
    return (
      <div className="text-center py-8">
        <h2 className="text-2xl font-bold text-foreground">
          No products found in leisure collection.
        </h2>
        {user?.role === "VENDOR" && (
          <p className="text-muted-foreground mt-2">
            Add some leisure products to see them here.
          </p>
        )}
      </div>
    );
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(displayCollections.leisure.length / ITEMS_PER_PAGE)
  );
  const safeCurrentPage = Math.min(currentPage, totalPages);
  const startIndex = (safeCurrentPage - 1) * ITEMS_PER_PAGE;
  const currentProducts = displayCollections.leisure.slice(
    startIndex,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="text-sm text-muted-foreground text-center mt-4">
        ...
        {Math.min(
          startIndex + ITEMS_PER_PAGE,
          displayCollections.leisure.length
        )}{" "}
        of {displayCollections.leisure.length} products
      </div>
    </div>
  );
};

export default VendorLeisureCollection;
